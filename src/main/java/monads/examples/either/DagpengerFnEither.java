...
import monads.examples.either.DagpengeSoknad.Avslag;
import monads.examples.either.DagpengeSoknad.Dagpenger;
...
public interface DagpengerFnEither extends Function<List<Double>, Either<Avslag, Dagpenger>> {
    ...
    DagpengerFnEither dagpengerFn = salaryList -> {
        ...
        if (!hasMinimumSalary) return Either.left(new Avslag("Har ikke tjent nok"));
